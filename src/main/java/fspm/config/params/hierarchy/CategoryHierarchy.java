...
public class CategoryHierarchy extends Hierarchy {
	...
	/**
	 * If parameters are stored in a flat structure, get and set parameter methods
	 * will
	...
	public ParamCategory getCategoryWithParam(String paramKey) {
		...
		throw new KeyNotFoundException(paramKey,
				String.format("Could not find parameter '%s' in any category within group '%s'", paramKey,
						super.getGroupKey()));
	}
	...
	private void validateFlattenedAccess(String key) {
		...
		if (useFlattenedCategories) {
			 ...
			 * {@link #setCategoryContext(String)}.
			 * FIXME: getCategoryWithParam(key) may be acting as a middle-man
			 * 
			 * Purpose: to find the category containing the key and set as the
			 * categoryContext, such that getters can
